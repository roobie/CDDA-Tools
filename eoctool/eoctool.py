...
class Condition:
    ...
    @staticmethod
    def u_has_effect(
        effect_id: str, intensity: Optional[int] = None, bodypart: Optional[str] = None
    ) -> Dict[str, Any]:
...
class Effect:
    ...
    @staticmethod
    def u_spawn_item(
        item_id: Union[str, Dict], count: Union[int, Dict] = 1
    ) -> Dict[str, Any]:
    ...
    @staticmethod
    def u_location_variable(
        var_name: Union[str, Dict],
        x_adjust: int = 0,
        y_adjust: int = 0,
        min_radius: int = 0,
        max_radius: int = 0,
    ...
    def u_run_inv_eocs(
        mode: str,
        search_data: Optional[List[Dict]] = None,
        true_eocs: Optional[List[str]] = None,
    ) -> Dict[str, Any]:
    ...
    def if_then_else(
        condition: Dict,
        then_effect: Union[Dict, List[Dict]],
        else_effect: Optional[Union[Dict, List[Dict]]] = None,
    ) -> Dict[str, Any]:
...
class EOC:
    ...
    def __init__(self, eoc_id: str):
        self.data: Dict[str, Any] = {"type": "effect_on_condition", "id": eoc_id}
...
if __name__ == "__main__":
    ...
    eoc1 = (
        EOC("EOC_stamina_check")
        .recurring("1 seconds")
        .condition(Condition.math("u_val('stamina') == 500"))
        .effect(Effect.u_add_morale("morale_feeling_good", 10))
        .false_effect(Effect.u_message("Low stamina", "bad"))
    )
    print(eoc1.to_json())
    print()
    ...
    eoc2 = (
        EOC("EOC_spell_event_test")
        .event("character_casts_spell")
        .effect(
            Effect.copy_var(Variable.context_val("spell"), Variable.global_val("key1")),
    ...
    eoc3 = EOC("EOC_I_NEED_AN_AR15").effect(
        Effect.run_eocs(
            "EOC_GIVE_A_GUN",
            variables={"gun_name": "ar15_223medium", "amount_of_guns": "5"},
        )
    )
    print(eoc3.to_json())
    print()
    ...
    eoc5 = EOC("EOC_strength_check").effect(
        Effect.if_then_else(
    ...
    eoc6 = EOC("EOC_teleport_test").effect(
        Effect.u_location_variable(Variable.u_val("tele_test"), x_adjust=1, y_adjust=1),
